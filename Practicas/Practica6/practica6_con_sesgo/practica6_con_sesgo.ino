...
float b_next = 0;
float b_act = 0;
float Ad[3][3] = {{1, 0.02,0},{-1.2250, 0.9475,0},{0,0,1}} ;
float Cd[2][3] = {{1,0,0},{0,1,1}};
float L[3][2] = {{0.4540,0.0472},{-1.2130,0.2293},{0.0617,0.1843}};
...
void loop() {
  ...
  float w = (g.gyro.x) * 180 / pi + 10;    
  thita_next = Ad[0][0] * thita_act + Ad[0][1] * w_act + L[0][0] * (thita_medido - thita_act) + L[0][1] * (w - b_act - w_next);
  w_next = Ad[1][0] * thita_act + Ad[1][1] * w_act + L[1][0] * (thita_medido - thita_act) + L[1][1] * (w - b_act - w_next);
  b_next = Ad[2][2] * b_act + L[2][0] * (thita_medido - thita_act) + L[2][1] * (w - b_act - w_next);
  b_act = b_next;
  w_act = w_next;  
  thita_act = thita_next;

  matlab_send(thita_medido, thita_next,w_next,w,b_next);
